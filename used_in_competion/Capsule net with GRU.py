...
train= pd.read_csv('train_prepro.csv')
...
filepath="capsule_prero.best.{}.hdf5"
...
def train_and_validate_model(x_tra, y_tra, x_val, y_val, i):
    model = get_model()
    checkpoint = ModelCheckpoint(filepath.format(i), monitor='val_loss', verbose=1, save_best_only=True, mode='min')
...
shit.to_csv("capsuleNet_prepro_oofs.csv")
...
submission.to_csv('capsuleNet_prepro_10foldave.csv', index=False)
